...
import static com.paint.global.utils.Utils.paintLine;
...
public class LineBrush extends Brush {
    ...
    public LineBrush(FieldPanel field) {
        super(field);
    }

    public LineBrush(FieldPanel field, int brushSize) {
    ...
    public void paintTargets(Graphics g) {
        if (targets.size() != 2) {
            ...
            paintLine(targets.get(0), targets.get(1), brushSize, getDrawingColor(), g);
        }
    }
    ...
    public void paintMarkers(Point mousePosition, Graphics g) {
        if (targets.size() > 1) {
            ...
            paintLine(targets.getFirst(), mousePosition, brushSize, getDrawingColor(), g);
        } else {
